...
export const fillAndStrokeText = text => (ctx, offset) => {
  const textMetrics = measureText(text.style, text.weight, text.size, text.family, text.baseline, text.align, text.textContent);
  ...
  while (textContent !== '' && width > text.maxWidth) {
    ...
    const textMetrics =  measureText(text.style, text.weight, text.size, text.family, text.baseline, text.align, textContent);
